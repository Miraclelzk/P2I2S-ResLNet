...
from util.util import AverageMeter, save_checkpoint, count_prompt_parameters
...
from models.voxelsnn import VOXELSNN
...
def point_cloud_to_voxel(point_cloud, voxel_size, voxel_shape):
    # 获取批次大小和点的数量
    B, N, _ = point_cloud.shape

    # 计算点云数据中的最小和最大坐标
    min_coords = torch.min(point_cloud, dim=1)[0].reshape(B, 1, 3)
    max_coords = torch.max(point_cloud, dim=1)[0].reshape(B, 1, 3)
    ...
    # 计算每个体素的尺寸
    voxel_step = (max_dim.repeat(1, 1, 3)) / torch.tensor(voxel_shape, dtype=torch.float32).cuda()

    # 初始化体素网格
    voxel_grid = torch.zeros((B,) + voxel_shape, dtype=torch.bool).cuda()

    # 将点云中的点映射到体素网格中
    grid_center = torch.tensor(voxel_shape, dtype=torch.float32).cuda() / 2
    voxel_indices = ((point_cloud - center_coords) / voxel_step + grid_center).long()
    voxel_indices = torch.clamp(voxel_indices, torch.tensor((0, 0, 0)).cuda(), torch.tensor(voxel_shape).cuda() - 1)
    
    # 为每个批次中的点设置体素值为 True
    for b in range(B):
        voxel_grid[b, voxel_indices[b, :, 0], voxel_indices[b, :, 1], voxel_indices[b, :, 2]] = True
...
#     # 计算每个体素的尺寸
#     voxel_step = (max_coords - min_coords) / torch.tensor(voxel_shape, dtype=torch.float32).cuda()

#     # 初始化体素网格
#     voxel_grid = torch.zeros((B,) + voxel_shape, dtype=torch.bool).cuda()

#     # 将点云中的点映射到体素网格中
#     voxel_indices = ((point_cloud - min_coords) / voxel_step).long()
#     voxel_indices = torch.clamp(voxel_indices, torch.tensor((0, 0, 0)).cuda(), torch.tensor(voxel_shape).cuda() - 1)
        ...
        if args.evaluate and (epoch_log % args.eval_freq == 0):
            ...
            if main_process():
                if args.data_name == 'modelnet' or args.data_name == 'scanobjectnn' or args.data_name == 'scanobjectnn_hardest':
                    ...
                    logger.info('Best Accuracy: %.3f' % (best_acc_cls))
...
def train_cls(train_loader, model, optimizer, scheduler, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        input_pc = train_transforms(input_pc) # B N 3

        voxel_size = args.voxel_size
        voxel_shape = tuple(args.voxel_shape)

        input_voxel = point_cloud_to_voxel(input_pc, voxel_size, voxel_shape)
        input_voxel = input_voxel.float().unsqueeze(1)

        # B 1 32 32 32
        # forward the model
        output = model(input_voxel) # B 40
...
def validate_cls(val_loader, model):
    ...
    phi = -0.35
    v_theta, v_phi = np.meshgrid(theta[:3], phi)
