...
from architecture import DeepGCN_SNN
...
if __name__ == '__main__':
    ...
    elif opt.data_name == 'scanobjectnn':

        train_loader = DataLoader(ScanObjectNN(data_dir=opt.data_dir, partition='train', num_points=opt.num_points),
                                num_workers=8, batch_size=opt.batch_size, shuffle=True, drop_last=True)
        test_loader = DataLoader(ScanObjectNN(data_dir=opt.data_dir, partition='test', num_points=opt.num_points),
                                num_workers=8, batch_size=opt.test_batch_size, shuffle=True, drop_last=False)

        logging.info('===> Loading ScanObjectNN from {}. number of classes equal to {}'.format(opt.data_dir, opt.n_classes))

    else:
        raise Exception('Dataset not supported yet'.format(opt.data_name))

    logging.info('===> Loading the network ...')
    model = DeepGCN_SNN(opt)
    if opt.T != 1:
